...
export default function AdminBlogEditPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/admin/blog/${id}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
      .then((res) => res.json())
      .then((data) => {
        const p = data.post || data.data?.post || data;
        if (p) {
          setPost(p);
          setForm({
            title: p.title || '',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#fcf9f8' }}>
        <div className="text-center">
          <span className="material-symbols-outlined text-6xl text-[#5d5e64] mb-4">article</span>
          <p className="text-[#45474b] text-lg">Blog post not found</p>
          <button onClick={() => router.push('/admin/blog')} className="mt-4 px-6 py-2 rounded-full bg-[#2d666d] text-white font-semibold">
            Back to Blog
          </button>
        </div>
      </div>
  ...
  return (
    <AuthGuard>
      <Head>
        <title>Edit {post.title} - Dreamy Life Admin</title>
      </Head>
      <style>{`
      ...
      <DesktopHeader title="Edit Blog Post" onMenuClick={() => setDrawerOpen(true)} avatarUrl={user?.info?.avatarUrl || ''} />
      ...
      <header className="md:hidden flex justify-between items-center px-6 py-5 sticky top-0 z-40 bg-white/60 backdrop-blur-xl border-b border-white/30">
        ...
        <h1 className="text-lg font-extrabold tracking-tight text-[#1c1b1b]">Edit Blog Post</h1>
        <div className="w-10" />
      </header>
      ...
      <main className="pt-20 md:pt-28 pb-10 md:pb-20 px-6 max-w-[700px] mx-auto min-h-screen">
        {success && (
          <div className="glass-panel rounded-2xl p-8 text-center mb-6">
            ...
            <p className="text-lg font-bold text-[#1c1b1b]">Blog post updated!</p>

...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/auth/forgot-password/request`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.error?.message || t('failedToSendOtp'));
        setLoading(false);
        return;
      }

      router.push(`/forgot-password/verify?phone=${encodeURIComponent(phoneNumber)}`);
    } catch (err) {
      ...
      >
        <div className="w-full max-w-[480px] bg-white/40 backdrop-blur-[20px] rounded-[3rem] p-8 md:p-12 flex flex-col items-center relative z-10 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-white/30">
          ...
          <div className="w-20 h-20 mb-8 rounded-full overflow-hidden shadow-[0_10px_20px_rgba(0,0,0,0.05)] bg-white/50 backdrop-blur-md p-1 border border-white/40 flex items-center justify-center">
            <span className="material-symbols-outlined text-[#5d5e64] text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>lock_reset</span>
          </div>

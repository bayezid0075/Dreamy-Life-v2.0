...
export default function VerifyOtpPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/auth/forgot-password/verify`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.error?.message || t('invalidOtp'));
        setLoading(false);
        return;
      }
      ...
      >
        <div className="w-full max-w-[480px] bg-white/40 backdrop-blur-[20px] rounded-[3rem] p-8 md:p-12 flex flex-col items-center relative z-10 shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-white/30">
          ...
          <div className="w-20 h-20 mb-8 rounded-full overflow-hidden shadow-[0_10px_20px_rgba(0,0,0,0.05)] bg-white/50 backdrop-blur-md p-1 border border-white/40 flex items-center justify-center">
            <span className="material-symbols-outlined text-[#5d5e64] text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
          </div>

          {/* Header */}
          <div className="text-center mb-8 w-full">
            <h1 className="text-[32px] font-bold text-[#1c1b1b] mb-2">{t('verifyOtp')}</h1>
            <p className="text-[16px] text-[#45474b]">{t('enterOtpSentTo')} {phoneNumber}</p>
          </div>
          ...
          <form className="w-full flex flex-col gap-5" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-1.5">
              <div className="rounded-full px-6 py-4 flex items-center gap-3 transition-all bg-white/50 backdrop-blur-[10px] border border-white/40">
                <span className="material-symbols-outlined text-[#76777b]">pin</span>
                <input
                  className="bg-transparent border-none focus:ring-0 p-0 w-full text-[16px] text-[#1c1b1b] placeholder:text-[#c6c6cb] outline-none text-center tracking-[0.5em] font-mono"
                  placeholder="000000"
                  type="text"
                  maxLength={6}
                  value={otpCode}
          ...
          <div className="mt-6 text-center">
            <p className="text-[14px] text-[#45474b]">
              {t('didntReceiveCode')}{' '}
              <button
                onClick={() => router.push('/forgot-password')}
                className="text-[#2d666d] font-semibold hover:underline decoration-[#2d666d]/50 underline-offset-4"
              >
                {t('resendOtp')}
              </button>
            </p>

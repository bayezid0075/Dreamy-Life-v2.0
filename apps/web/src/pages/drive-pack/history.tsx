...
const KNOWN_OPERATORS: Record<string, { name: string; shortName: string; color: string; bg: string }> = {
...
function getOperatorInfo(opId: string) {
  if (KNOWN_OPERATORS[opId]) return KNOWN_OPERATORS[opId];
  const colorIdx = hashString(opId) % FALLBACK_COLORS.length;
  const bg = FALLBACK_COLORS[colorIdx];
  return { name: opId, shortName: opId.slice(0, 2).toUpperCase(), color: '#ffffff', bg };
}
...
export default function DrivePackHistoryPage() {
  ...
  useEffect(() => {
    ...
    const handleRouteChange = (url: string) => {
      if (url === '/drive-pack/history') fetchOrders();
    };
  ...
  return (
    <AuthGuard>
      <Head>
        <title>Drive Pack History - Dreamy Life</title>
      </Head>
      <style>{`
      ...
      >
        ...
        <header className="md:hidden flex justify-between items-center px-6 py-5 sticky top-0 z-40 bg-white/60 backdrop-blur-xl border-b border-white/30">
          <Link href="/drive-pack" className="w-10 h-10 rounded-full flex items-center justify-center bg-white/50 border border-white/40 shadow-sm text-[#45474b]">
            <span className="material-symbols-outlined">arrow_back</span>
          </Link>
          <h1 className="text-lg font-extrabold tracking-tight text-[#1c1b1b]">Drive Pack History</h1>
          <button
        ...
        <header className="hidden md:flex fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-[30px] border-b border-white/40 shadow-[0_20px_40px_rgba(0,0,0,0.06)] px-6 py-4 justify-between items-center">
          <Link href="/drive-pack" className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/40 transition-colors text-[#45474b]">
            <span className="material-symbols-outlined">arrow_back</span>
          </Link>
          <div className="text-lg font-extrabold tracking-tight text-[#1c1b1b]">Drive Pack History</div>
          <button
            onClick={() => fetchOrders(true)}
            disabled={refreshing}
          ...
          {orders.length === 0 ? (
            <div className="text-center py-20 animate-fade-in">
              <div className="w-20 h-20 rounded-full bg-white/50 backdrop-blur-md flex items-center justify-center mx-auto mb-6 border border-white/30">
                <span className="material-symbols-outlined text-[#45474b] text-3xl">inventory_2</span>
              </div>
              <h3 className="text-xl font-bold text-[#1c1b1b] mb-2">No Drive Packs Yet</h3>
              <p className="text-sm text-[#45474b] mb-6">Your drive pack history will appear here</p>
              <Link
                href="/drive-pack"

...
const OPERATOR_COLORS: Record<string, { color: string; bgColor: string }> = {
  ...
  ar: { color: '#ed1c24', bgColor: '#fde6e7' },
};
...
export default function DrivePackHistoryScreen() {
  ...
  const loadOrders = async (pageNum: number = 1, append: boolean = false) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/recharge/drive-pack-orders?page=${pageNum}&limit=20`);
      ...
      console.error('Failed to load drive pack orders', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <View style={styles.container}>
      <AuroraBackground />
      <TopBar showBack title="Drive Pack History" showNotification={false} showSearch={false} />
      ...
      >
        {orders.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>No Drive Packs Yet</Text>
            <Text style={styles.emptySubtitle}>
              Your drive pack history will appear here.
            </Text>
            <TouchableOpacity
              onPress={() => router.push('/drive-pack')}
              style={styles.browseBtn}
              activeOpacity={0.8}
            >
              <Text style={styles.browseBtnText}>Browse Drive Packs</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>

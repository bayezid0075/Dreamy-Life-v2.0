...
export default function WalletHistoryScreen() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      const res = await authFetch(`${API_URL}/wallet/transactions?type=wallet&filter=${timeRange}`);
  ...
  const getTypeInfo = (type: string) => {
    if (type === 'wallet_credit') return { icon: '↓', color: '#2d666d', bg: '#e9fdff', label: 'Credit', positive: true };
  ...
  return (
    <View style={styles.container}>
      <AuroraBackground />
      <TopBar showBack title="Wallet History" showNotification={false} showSearch={false} />

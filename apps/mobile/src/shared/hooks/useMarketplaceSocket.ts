...
export async function connectMarketplaceSocket(): Promise<Socket> {
  if (marketplaceSocket?.connected) return marketplaceSocket;

  const token = await AsyncStorage.getItem('accessToken');
  if (!token) throw new Error('No access token');

  marketplaceSocket = io(`${WS_URL}/marketplace`, {
    auth: { token },
...
export function useMarketplaceSocket(handlers: {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        const socket = await connectMarketplaceSocket();
        if (!mounted) return;
        ...
        console.error('[Marketplace Socket] Setup failed:', err);
      }
    };

...
class RegisterSerializer(serializers.Serializer):
    ...
    referred_by = serializers.CharField(required=True, allow_blank=False, trim_whitespace=True)
    ...
    def validate_referred_by(self, v):
        code = (v or "").strip() if isinstance(v, str) else ""
        if not code:
            raise serializers.ValidationError("Referral code is required")
        if not UserInfo.objects.filter(own_refercode__iexact=code).exists():
            raise serializers.ValidationError("No user found with this referral code")
        return code

    def create(self, validated_data):
        referral_code = validated_data.pop("referred_by")
        parent_info = UserInfo.objects.get(own_refercode__iexact=referral_code)

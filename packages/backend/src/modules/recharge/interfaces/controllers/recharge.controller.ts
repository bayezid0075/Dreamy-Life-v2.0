...
export class RechargeController {
  ...
  async createRecharge(
    @Req() req: Request,
    @Body() body: { phoneNumber: string; operator: string; connectionType: string; amount: number },
  ) {
    const userId = this.extractUserId(req);
    this.logger.log(`POST /recharge/create user=${userId} phone=${body.phoneNumber} operator=${body.operator} amount=${body.amount} type=${body.connectionType}`);

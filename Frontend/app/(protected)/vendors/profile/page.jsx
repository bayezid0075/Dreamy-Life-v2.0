...
export default function VendorProfilePage() {
  ...
  const fetchVendorProfile = async () => {
    try {
      setLoading(true);
      // Try to get vendor profile - adjust endpoint based on your backend
      const response = await axios.get("/api/vendors/vendors/");
      // Assuming the API returns a list, get the first one or filter by current user
      ...
      // If vendor doesn't exist (404), that's okay - they can create one
      // For other errors, show a message but don't block the form
      if (error.response?.status === 404) {
        // Vendor doesn't exist yet, allow creation
      } else if (error.response?.status !== 404) {
        toast.error("Failed to load vendor profile", {
          description: error.response?.data?.detail || "You may need to create a vendor profile first",
        });
      }
    } finally {
      setLoading(false);

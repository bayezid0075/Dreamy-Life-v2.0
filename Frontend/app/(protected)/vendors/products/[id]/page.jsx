...
import { Card, Button, Input, Select, Textarea, Upload, Spinner } from "components/ui";
...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/vendors/products/${productId}/`);
      ...
      setValue("reseller_mrp_price", product.reseller_mrp_price || "");
      setValue("delivery_charge_inside_dhaka", product.delivery_charge_inside_dhaka || "");
      setValue("delivery_charge_outside_dhaka", product.delivery_charge_outside_dhaka || "");
      setValue("vat", product.vat || "0.00");
      ...
      if (product.sub_categories && product.sub_categories.length > 0) {
        const subCatIds = product.sub_categories.map(sc => 
          typeof sc === 'object' ? sc.id.toString() : sc.toString()
        );
        setValue("sub_categories", subCatIds);
      }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await axios.put(`/api/vendors/products/${productId}/`, formData, {
        headers: {
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  {existingImages.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-gray-700 dark:text-dark-300 sm:text-sm">
                        Existing Images
                      </p>

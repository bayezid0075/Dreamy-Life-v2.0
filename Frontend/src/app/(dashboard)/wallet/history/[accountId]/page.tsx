...
function filterByRange(
  ...
  return transactions
    .filter((t) =>
      isWithinInterval(parseISO(t.created_at), {
        start: dates.start,
        end: dates.end,
      }),
    )
...
export default function WalletHistoryPage() {
  ...
  const filtered = useMemo(
    () => filterByRange(transactions, timeRange),
  ...
  return (
    <div className="min-h-dvh bg-slate-50 dark:bg-slate-950 pb-6 md:pb-8">
      <div className="px-3 py-4 sm:px-4 sm:py-5 md:px-0 md:py-0 space-y-4 sm:space-y-5 md:space-y-6">
        ...
        <Card
          className={`relative overflow-hidden border-0 shadow-2xl shadow-violet-500/20 bg-gradient-to-br ${config.gradient}`}
        >

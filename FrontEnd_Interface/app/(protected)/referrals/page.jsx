...
import { Card, Button, Accordion, AccordionItem, AccordionButton, AccordionPanel } from "components/ui";
...
export default function Referrals() {
  ...
  // Generate sample data for all time periods
  ...
  const currentData = useMemo(() => {
    const periodData = allTimeData.find(
      (p) => p.period.id === selectedPeriod.id,
    );
    return periodData?.data || [];
  }, [allTimeData, selectedPeriod]);
  ...
  // Generate users for each level
  const levelUsers = useMemo(() => {
    return currentData.map((levelData) => ({
      level: levelData.level,
      users: generateUsersForLevel(levelData.level, levelData.count),
    }));
  }, [currentData]);
  ...
  return (
    <Page title="Referrals">
      <div className="transition-content w-full px-(--margin-x) pt-5 lg:pt-6">
        ...
        <div className="mb-6">
          <div className="dark:bg-dark-800 flex flex-wrap gap-2 rounded-lg border border-gray-200 bg-white p-1 dark:border-dark-600 sm:inline-flex sm:flex-nowrap">
            {TIME_PERIODS.map((period) => (
              <button
                key={period.id}
                onClick={() => setSelectedPeriod(period)}
                className={clsx(
                  "flex-1 rounded-md px-3 py-2 text-xs font-medium transition-all sm:flex-initial sm:px-4 sm:text-sm",
        ...
        <Card className="dark:border-dark-600 border border-gray-200 p-6">
          <div className="mb-4">
            ...
            <p className="dark:text-dark-400 mt-1 text-sm text-gray-600">
              Showing referral counts for all 10 levels ({selectedPeriod.label})
            </p>
          </div>
        ...
        <Card className="dark:border-dark-600 mt-6 border border-gray-200">
          <div className="dark:bg-dark-800 border-b border-gray-200 bg-gray-50 px-4 py-4 dark:border-dark-600 sm:px-6">
            <h3 className="dark:text-dark-50 text-lg font-semibold text-gray-900">
              Level Details
            </h3>
          </div>
          <div className="p-4 sm:p-6">
            <Accordion defaultValue={currentData.find((d) => d.count > 0)?.level || 1}>
              {Array.from({ length: 10 }, (_, i) => {
                ...
                const percentage =
                  totalReferrals > 0
                    ...
                    : "0.0";
                const users = levelUsers.find((lu) => lu.level === level)?.users || [];

                return (
                  <AccordionItem
                    key={level}
                    value={level}
                    className="dark:border-dark-600 mb-3 overflow-hidden rounded-lg border border-gray-200 bg-white transition-all dark:bg-dark-800"
                  >
                    {({ open }) => (
                      <>
                        <AccordionButton
                          className={clsx(
                            ...
                            count > 0 && "hover:bg-purple-50 dark:hover:bg-purple-900/10",
                          )}
                        >
                          <div className="flex w-full items-center justify-between">
                            <div className="flex flex-1 items-center gap-3 sm:gap-4">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2 sm:gap-3">
                                  ...
                                  {count > 0 && (
                                    <span className="inline-flex rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-800 dark:bg-green-900/20 dark:text-green-400 sm:hidden">
                                      Active
                                    </span>
                                  )}
                                </div>
                                <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-dark-400">
                            ...
                            <div className="hidden items-center gap-4 sm:flex">
                              <div className="flex items-center gap-2">
                                <div className="h-2 w-24 rounded-full bg-gray-200 dark:bg-dark-700">
                                  <div
                              ...
                              {count > 0 ? (
                                ...
                                <span className="inline-flex rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-800 dark:bg-dark-700 dark:text-dark-300">
                                  Inactive
                                </span>
                              )}
                            </div>
                        ...
                        <AccordionPanel className="dark:bg-dark-900 border-t border-gray-200 bg-gray-50 dark:border-dark-600">
                          {count > 0 ? (
                            <div className="p-4 sm:p-6">
                              <div className="mb-4">
                                <p className="dark:text-dark-300 text-sm text-gray-600">
                                  {count} {count === 1 ? "user" : "users"} in Level {level}
                                </p>
                              </div>
                                ...
                                {users.map((user) => (
                                  ...
                                  >
                                    <div className="space-y-3">
                                      ...
                                      {/* Phone Number */}
                                      <div className="flex items-start gap-2">
                                        <PhoneIcon className="dark:text-dark-400 mt-0.5 size-4 shrink-0 text-gray-500" />
                                        <div className="min-w-0 flex-1">
                                          <p className="dark:text-dark-400 text-xs text-gray-500">
                                            Phone
                                          </p>
                                          <p className="dark:text-dark-100 mt-0.5 truncate text-sm font-medium text-gray-900">
                                            {user.phoneNumber}
                                          </p>
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex gap-2 pt-2">
                                        ...
                                        <Button
                                          size="sm"
                                          variant="plain"
                                          className="flex-1 gap-2 bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-dark-700 dark:text-dark-300 dark:hover:bg-dark-600"
